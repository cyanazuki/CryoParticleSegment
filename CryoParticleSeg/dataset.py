...
class MicrographDataset(Dataset):
  ...
  def transform(self, image, mask):
    ...
    mask = torch.concat([mask, 1-mask], dim=0) # Remove this line if background is not consider.
...
class MicrographDatasetEvery(MicrographDataset):
  ...
  def transform(self, image, mask):
    ...
    masks = torch.concat([masks,1-masks], dim=1) # Remove this line if background is not consider.
